...
### Fill Qty Notice
An orange notice bar always appears above the buttons showing the active fill quantity:
> `Fill Qty: 50 units`

This reflects the Fill Qty configured in Settings. See [Fill Qty](#fill-qty) below.
...
| **Fill Qty** | 50 | Units to simulate purchasing from the AH order book. Range: 10–10,000 |
...
GAM prices reagents by simulating the purchase of **Fill Qty** units from the AH order book (default: **50**). Lower values reflect the cost of buying small batches; raise it if you typically buy in larger quantities.

Changing Fill Qty automatically **clears the price cache** so the next scan applies the new setting.

### Configuring Fill Qty
1. Open Settings (right-click minimap button).
2. Find the **Fill Qty** editbox. Type a number between 10 and 10,000.
3. Click **Apply & Close**.

The Strategy Detail window always shows the active Fill Qty in the orange notice bar above the buttons.
